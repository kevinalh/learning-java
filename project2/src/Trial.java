...
public class Trial extends MultipleItemAbstract
{
     ...
     */
    @Override
    public GeneralValue getMaxLeftWrist(int dimension)
    {
        GeneralValue maxGeneralValueSoFar = new GeneralValue(Double.MIN_VALUE);
        GeneralValue currentValue = null;
        for (State current : stateList)
        {
            currentValue = current.getLeftWrist().getDimValue(dimension);
            if (currentValue.isGreaterThan(maxGeneralValueSoFar))
            {
                maxGeneralValueSoFar = currentValue;
            }
        }
        return maxGeneralValueSoFar;
    }
     ...
     */
    @Override
    public GeneralValue getMinLeftWrist(int dimension)
    {
        GeneralValue minGeneralValueSoFar = new GeneralValue(Double.MAX_VALUE);
        GeneralValue currentValue = null;
        for (State current : stateList)
        {
            currentValue = current.getLeftWrist().getDimValue(dimension);
            if (currentValue.isLessThan(minGeneralValueSoFar))
            {
                minGeneralValueSoFar = currentValue;
            }
        }
        return minGeneralValueSoFar;
    }
     ...
     */
    @Override
    public GeneralValue getAverageLeftWrist(int dimension)
    {
        double average = 0;
        int validValues = 0;
        ...
        return new GeneralValue(average / validValues);
    }
